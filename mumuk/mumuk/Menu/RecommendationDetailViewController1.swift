...
class RecommendationDetailViewController1: UIViewController {
    ...
    private func updateUIWithRankData() {
        ...
        if let rankLabel = view.subviews.first(where: { ($0 as? UILabel)?.text?.hasPrefix("1등") == true }) as? UILabel {
            rankLabel.text = "1등 \(rank.foodname)"
        }
...
extension RecommendationDetailViewController1: UIViewControllerTransitioningDelegate {
