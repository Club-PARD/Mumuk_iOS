...
class PreferViewController4 : UIViewController{
    ...
    let foodButton1 : UIButton = {
        ...
        if let image = UIImage(named: "hot"){
            let size = CGSize(width: 53, height: 66)
        ...
        var text = AttributedString.init("빨간 맛")
    ...
    let foodButton2 : UIButton = {
        ...
        if let image = UIImage(named: "no_hot"){
            let size = CGSize(width: 87, height: 80)
            UIGraphicsBeginImageContextWithOptions(size, false, 0.0)
            image.draw(in: CGRect(origin: .zero, size: size))
            let scaledImage = UIGraphicsGetImageFromCurrentImageContext()
            UIGraphicsEndImageContext()
        ...
        var text = AttributedString.init("안 빨간 맛")
        text.foregroundColor = #colorLiteral(red: 0.7254901961, green: 0.7254901961, blue: 0.7254901961, alpha: 1)
        text.font = UIFont(name: "Pretendard-Bold" , size: 20)
        config.attributedTitle = text
    ...
    @objc func buttonClicked1(_ sender: UIButton) {
        ...
        updateButtonConfiguration(button: sender, isSelected: sender.isSelected, text: "빨간 맛")
        updateButtonConfiguration(button: foodButton2, isSelected: false, text: "안 빨간 맛")
    ...
    @objc func buttonClicked2(_ sender: UIButton) {
        ...
        updateButtonConfiguration(button: foodButton1, isSelected: false, text: "빨간 맛")
        updateButtonConfiguration(button: sender, isSelected: sender.isSelected, text: "안 빨간 맛")
    ...
    func updateButtonConfiguration(button: UIButton, isSelected: Bool, text: String) {
        ...
        if text == "빨간 맛" {
            if let image = UIImage(named:"hot") {
                newConfig?.image = resizeImage(image: image, targetSize: CGSize(width: 53, height: 66))
            }
        } else if text == "안 빨간 맛" {
            if let image = UIImage(named:"no_hot") {
                newConfig?.image = resizeImage(image: image, targetSize: CGSize(width: 87, height: 80))
            }
        }
    ...
    @objc func skipToNext(){
        if foodButton1.isSelected == false && foodButton2.isSelected == false {
            let preferVC = PreferViewController5()
